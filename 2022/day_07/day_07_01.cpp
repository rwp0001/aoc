...
class File {
  public:
    ...
    File( std::string Input ){
      // 14848514 b.txt
      std::string S1,S2;
      Size = std::stoi(Input.substr(0,Input.find(' ')));
...
class Directory {
  public:
    ...
    Directory( std::string Input , Directory ParentDirectory ){
      if( Input.substr( 0, Input.find( ' ' ) ) != "dir" ) throw new std::logic_error( "Not A Directory." );
      Name = Input.substr( Input.find( ' ' ) + 1 );
    ...
    int Size() {
      int r = 0;
      for( File F : Files ) r+= F.Size;
      for( Directory D : Directories ) r+= D.Size();
      return r;
    }

    Directory *cd( std::string Path ) {
      if( Path == ".." ) return ParentDir;
      for( Directory *D : Directories )  if( D.Name == Path ) return D;
...
void ReadFile( std::string filename ) {
    ...
    try {
      if (myfile.is_open()){
        while ( getline( myfile, line ) ) Listing.push_back( line );
...
void ProcessList(){
  ...
  Listing.pop_front(); // root is already made.

  for(std::string Line : Listing ) {

    if( Line[0] == '$' ) { // doing a command.
      switch ( Line[2] ) {
        case 'c': // cd
          Directory *Temp = ParentDir;
          ParentDir = CurrentDir;
          CurrentDir = ParentDir.cd( Line.substr(5) );
          break;
        ...
        default:
          throw new std::logic_error( "Unknown Command." );
          break;
      }
