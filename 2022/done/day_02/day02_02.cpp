...
class RPS {
    ...
    RPS( std::string Line ) {
        ...
        switch ( Result ){
            case 'X': // Lose
            if( TheirMove == 'C' ) MyMove = 'Y';
            if( TheirMove == 'B' ) MyMove = 'X';
            if( TheirMove == 'A' ) MyMove = 'Z';
            break;

            case 'Y': // Draw
            if( TheirMove == 'A' ) MyMove = 'X';
            if( TheirMove == 'B' ) MyMove = 'Y';
            if( TheirMove == 'C' ) MyMove = 'Z';
            break;

            case 'Z': // Win
            if( TheirMove == 'C' ) MyMove = 'X';
            if( TheirMove == 'B' ) MyMove = 'Z';
            if( TheirMove == 'A' ) MyMove = 'Y';
            break;
        ...
        if( TheirMove == 'A' & MyMove == 'X' || TheirMove == 'B' & MyMove == 'Y' || TheirMove == 'C' & MyMove == 'Z' ) {
            Score += 3;
            TotalScore += Score;
            std::cout << "Draw: " << Line << " Score: " << Score << " Total: " << TotalScore << "\n";
            return;
        }

        // Rock defeats Scissors
        if( MyMove == 'X' & TheirMove == 'C' ){
            Score += 6;
            TotalScore += Score;
            Win = true;
            std::cout << "Winner: " << Line << " Score: " << Score << " Total: " << TotalScore << "\n";
            return;
        }

        // Scissors defeats Paper
        if( MyMove == 'Z' & TheirMove == 'B' ){
            Score += 6;
            TotalScore += Score;
            Win = true;
            std::cout << "Winner: " << Line << " Score: " << Score << " Total: " << TotalScore << "\n";
        ...
        // Loser
        TotalScore += Score;
        std::cout << "Loser: " << Line << " Score: " << Score << " Total: " << TotalScore << "\n";
...
void ReadFile( std::string filename ) {
    ...
    //Data.sort();
    std::cout <<  "Total Score: " << RPS::TotalScore  << "\n";
}
