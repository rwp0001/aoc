...
std::array<std::list<char>,10> Stacks;
std::list<std::string> Data;

void GetLayers( std::string Input ){
    for ( int i = 0; i < 9; i++ ) if( Input[ 1 + ( i * 4 ) ] != 32 ) Stacks[ i+1 ].push_back( Input[ 1 + ( i * 4 ) ] );
}

void ReadStacks(){
        auto CurrentLine = Data.begin();
        for( int i = 0; i < 8; i++ ) CurrentLine++;
        do {
                GetLayers(*--CurrentLine);
        } while ( CurrentLine != Data.begin() );
}

void Move( std::string Input ){
        int Count = std::stoi( Input.substr( 5 , 2 ) );
        int From = std::stoi( Input.substr( Input.find( "from" ) + 5, 2 ) ); 
        int To = std::stoi( Input.substr( Input.find( "to" ) + 2 ) ); 
        
        std::list< char > Crane;

        //pickup
        for( int i = 0; i < Count; i++ ) {
                Crane.push_back( Stacks[From].back() );
                Stacks[From].pop_back();
        }
        // put down
        for( int i = 0; i < Count; i++ ) {
                Stacks[To].push_back( Crane.back() );
                Crane.pop_back();
        }


        //::cout << Input <<" | Count: " << Count << " From: " << From <<" To: " << To << std::endl;
}

void DoMoves(){
        auto CurrentLine = Data.begin();
        for( int i = 0; i < 10; i++ ) CurrentLine++;
        do {
                Move(*CurrentLine++);
        } while ( CurrentLine != Data.end() );
}
...
int main() {
    ...
    ReadStacks();
    DoMoves();
    for( int i = 1; i<10; i++ ) std::cout << Stacks[i].back();
    std::cout << std::endl;
