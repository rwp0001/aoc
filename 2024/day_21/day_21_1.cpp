...
A quick life-form scan by the ship's computer reveals that when the missing Historian teleported, 
...
and finally an empty gap followed by 0A. Visually, 
...
Unfortunately, the area outside the door is currently depressurized and nobody can go near the door. 
A robot needs to be sent instead.

The robot has no problem navigating the ship and finding the numeric keypad, 
but it's not designed for button pushing: it can't be told to push a specific button directly. 
...
a gap / ^ (up) / A (activate) on the first row and < (left) / v (down) / > (right) on the second row. 
...
and the A button causes the robot to briefly move forward, 
pressing the button being aimed at by the robotic arm.

For example, 
to make the robot type 029A on the numeric keypad, 
...
In total, 
there are three shortest possible sequences of button presses on this 
directional keypad that would cause the robot to type 029A: 
<A^A>^^AvvvA, 
<A^A^>^AvvvA, and 
<A^A^^>AvvvA.

Unfortunately, 
the area containing this directional keypad remote control is currently experiencing 
high levels of radiation and nobody can go near it. 
A robot needs to be sent instead.

When the robot arrives at the directional keypad, 
its robot arm is pointed at the A button in the upper right corner. 
After that, 
a second, 
different directional keypad remote control is used to control this robot 
(in the same way as the first robot, 
except that this one is typing on a directional keypad instead of a numeric keypad).

There are multiple shortest possible sequences of directional keypad button presses that would cause 
this robot to tell the first robot to type 029A on the door. 
...
Unfortunately, 
the area containing this third directional keypad remote control is currently full of Historians, 
...
here are all of the buttons that would be pressed on your directional keypad, 
...
even for an instant, 
the robot will panic unrecoverably. 
So, don't do that. 
All robots will initially aim at the keypad's A key, 
wherever it is.

To unlock the door, 
five codes will need to be typed on its numeric keypad. 
...
For each of these, 
here is a shortest sequence of button presses you could type to cause the desired code to be typed on the numeric keypad:
...
You'll need to make sure that for each of the five codes, 
...
complexity of the five codes can be found by calculating 68 * 29, 60 * 980, 68 * 179, 64 * 456, and 64 * 379. 
...
//#define SAMPLE 1
    ...
    #define ANSWER 126384
    //#define ANSWER "<A^A>^^AvvvA"
#else
...
class NumKeypad {
    ...
    friend std::ostream& operator << (std::ostream &s, const NumKeypad &k){
        s << "Key: ";     
        s << k.CurrentKey;
        return s; 
    }

    std::string Moves(std::string Passcode){
        std::string returnString = ""; 
        for( char c : Passcode){
            returnString.append(moveto(c));
        }
        return returnString;
    }

    std::string moveto(char Target){
        //are we there?
        if(CurrentKey == Target) return "A";
...
class DirKeypad {
    ...
    friend std::ostream& operator << (std::ostream &s, const DirKeypad &k){
        s << "Key: ";     
        s << k.CurrentKey;
        return s; 
    }
    ...
    private:
    const std::array<std::array<char,3>,2> keys = {{
        {' ','^','A'},
        {'<','v','>'}
    }};
...
int main()
{
    ...
    NumKeypad KP =  NumKeypad();
    DirKeypad DK = DirKeypad();

    for(std::string s : DataSet ){
        std::string k1 = KP.Moves(s);
        std::string k2 = DK.Moves(k1);
        std::string k3 = DK.Moves(k2); 
        std::cout << s << " | " << k1 << "\n"; 
        //std::cout << s << ": " << k3 << " ";
        int a = stoi( s.substr(0,3));
        //std::cout << a << " " << k3.length();
        a *= k3.length();
        //std::cout << " | " << a << "\n";
        Count += a;
    }
