...
but the second 6 occurs after the changes -1,-1,0,2. So, if you gave the monkey that sequence of changes, 
it would wait until the first time it sees that sequence and then immediately sell your hiding spot information at the current price, winning you 6 bananas.

Each buyer only wants to buy one hiding spot, 
so after the hiding spot is sold, 
the monkey will move on to the next buyer. 
If the monkey never hears that sequence of price changes from a buyer, 
the monkey will never sell, and will instead just move on to the next buyer.
...
There are many sequences of four price changes you could tell the monkey, but for these four buyers, 
the sequence that will get you the most bananas is -2,1,-1,3. 
...
std::vector<std::array<std::list<int>,10>> BestPrices;
std::list<std::array<int,3>> SqnDone;
...
std::array< int, 4 > GetSequence( int index, std::vector<int> &Data ){
    if( index < 4 ) throw new std::exception();
    std::array< int, 4 > ReturnValue;
    ReturnValue[0] = Data[index-4] - Data[index-3];
    ReturnValue[1] = Data[index-3] - Data[index-2];
    ReturnValue[2] = Data[index-2] - Data[index-1];
    ReturnValue[3] = Data[index-1] - Data[index];
    return ReturnValue;
}

bool SqnIsDone(std::array<int,3> Input ){
    for(std::array<int,3> Test :SqnDone ) if(Test == Input) return true;
    SqnDone.push_back(Input);
    return false;
}

bool HasSequence(std::array<int,4> Input, std::vector<int> &Data){
    for(int i = 4; i < Data.size(); i++ ){
        std::array<int,4> Test = GetSequence(i,Data);
        if(Test[0] == Input[0] & Test[1] == Input[1] & Test[2] == Input[2]) return true;
    } 
    return false;
}

int FirstOfSequence(std::array<int,4> Input, std::vector<int> &Data){
    for(int i = 4; i < Data.size(); i++ ){
        std::array<int,4> Test = GetSequence(i,Data);
        if(Test[0] == Input[0] & Test[1] == Input[1] & Test[2] == Input[2]) return i;
    }
    return -1;
}

void LoadPrices(int Count){
    for( int buyer : DataSet ) {
        std::vector<int> Offers;
        int Secret = SecretNumber(buyer);
        int p = Price(buyer);
        Offers.push_back(p);
        ...
        for(int i = 0; i<Count; i++) {
            ...
            p = Price(Secret);
            int d = p - Offers.front();
            Offers.push_back(p);
        }
        Prices.push_back(Offers);

        std::array<std::list<int>,10> BuyerBest;
        for(int i = 4; i<Offers.size(); i++) {
            std::array< int, 4 > Data = GetSequence(i,Offers);
            if(Data[3] < 0) continue;
            BuyerBest[Data[3]].push_back(i);
        }
        BestPrices.push_back(BuyerBest);
    }
...
int ProcessBuyer(int buyer){
    auto b_start = std::chrono::high_resolution_clock::now();
    std::cout << "Working Buyer: "<< buyer << std::endl;
    int Best = 0;
    for( int i = 9; i > 0; i--) { // count backwards
        for( int index : BestPrices[buyer][i] ) {
            std::array<int,4> Sqn = GetSequence(index,Prices[buyer]);
            if( SqnIsDone({Sqn[0],Sqn[1],Sqn[2]}) ) continue;
            int Score = Sqn[3];
            for( int buyer2 = 0; buyer2 < DataSet.size(); buyer2++ ) {
                if(buyer == buyer2) continue;
                int B2_index = FirstOfSequence(Sqn,Prices[buyer2]);
                if(B2_index == -1) continue;
                Score += GetSequence(B2_index,Prices[buyer2])[3];

            }
            if( Score > Best ) {           
                Best = Score;
                std::cout << "Buyer: " << buyer <<" New Best: " << Best << std::endl;
            }

        }
    }
    auto b_stop = std::chrono::high_resolution_clock::now();
    auto duration = std::chrono::duration_cast<std::chrono::microseconds>(b_stop - b_start);
    std::cout << "Buyer: " << buyer << " Time taken by function: " << duration.count() << " microseconds" << std::endl;
    return Best;
}
...
int main()
{
    ...
    for( int buyer = 0; buyer < DataSet.size(); buyer++ ) {
        int Score = ProcessBuyer(buyer);
        std::cout << "Sequences: " << SqnDone.size() << std::endl;
        if( Score > Count ) {           
            Count = Score;
            std::cout << "New Score: " << Count << std::endl;
        }
    }
